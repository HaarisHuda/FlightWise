...
const FlightPricePredictor = () => {
          ...
          {result !== null && (
            <Box sx={{ mt: 3, textAlign: 'center' }}>
              <Typography variant="h5" component="div" gutterBottom>
                Predicted Price: ₹{result.toLocaleString()}
              </Typography>
            </Box>
          )}
        </Box>
  ...
  const FlightPricePredictor = () => {
    ...
    const airlines = [
      'Delta Airlines', 'American Airlines', 'United Airlines', 'Southwest Airlines', 'Alaska Airlines'
    ];
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        // Convert the price from ₹ to $ using the formula ₹ * 80 * 3.5
        const priceInUSD = (data.predicted_price / 80 )* 3.5;
  
        setResult(priceInUSD);
      } catch (err) {
        setError(err.message);
      } finally {
        setLoading(false);
        ...
        <Paper elevation={3} sx={{ p: 4, mt: 4 }}>
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 3 }}>
            ...
            {result !== null && (
              <Box sx={{ mt: 3, textAlign: 'center' }}>
                <Typography variant="h5" component="div" gutterBottom>
                  Predicted Price: ${result.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </Typography>
              </Box>
            )}
          </Box>
